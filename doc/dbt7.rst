...
Both tests need to be run together to constitute a complete test run.  A
*Queries per Hour* score, calculated from the results of both the load and
performance tests, is reported at the end of a complete test.

The performance test is composed of several additional tests: a power test,
two data maintenance tests, and two throughput tests.
...
The TPC's TPC-DS Tools are not distributed as part of the DBT-7 source
code, but are available as a git submodule (see **Before getting started**
below) or can be downloaded from the TPC:
...
* Docutils - **rst2html5** is used to generate HTML from the reStructuredText
  reports.
* Touchstone Tools - **ts** is required by the `--stats` and `--profile`
  flags to collect system and database statistics and software profiles,
  and by *dbt7-report* to plot the collected statistics:
  https://gitlab.com/touchstone/touchstone-tools
* **sar**, **pidstat** - Used for collecting system and per process
...
This will generate the data files for a 1 GB scale factor database, then
run the load test, and the power and throughput tests with 4 streams,
against PostgreSQL and save the results of the test in `/tmp/results`.
...
The *dbt7-run* script will generate a *summary.rst* text file that can be
reviewed for the execution times for each of the tests, query times for
each of the queries in the power test, and the aggregates of the query
times from the throughput test streams.
...
2. Create a new query template file for any modification needed from the
   official templates, and reference it from the list file.  e.g. Copy
   `query23.tpl` into a `query_postgresql` directory next to
   `query_templates` and reference it as `../query_postgresql/query23.tpl`
   in `templates-pgsql.lst`.  While not required, it is important to keep
   the query number in the filename for proper recording and analysis with
   the DBT-7 kit.
...
The `pg_plan_advice` directory next to `query_templates` holds the
PostgreSQL templates with a line before each statement that sets
`pg_plan_advice.advice` to the plan advice of the fastest plan found for
that statement in a study on PostgreSQL 19 at scale factor 20 (see
`advice/pg19-sf20/README.rst` in the kit sources).  The `pgsqla` dialect
loads the extension and runs the set as a timed test, and the `pgsqlaea`
dialect also captures every plan with its advice feedback through
auto_explain::

    dbt7 run -d pgsqla -i "${DSHOME}/query_templates/templates-pgsqla.lst" \
        pgsql /tmp/results

The advised templates are generated by `tools/advise-templates.sh` from
the plain templates and one advice file per statement; change the advice
files and regenerate rather than editing the templates.
...
Query files can be created outside of the execution of the Power Test and
Throughput Test with the use of the *dbt7-generate-queries* script.  It is
primarily intended to be able to review queries generated by **dsqgen**
without having to run a Power Test or Throughput Test first.
...
Running the individual parts of the benchmark, i.e. the Power Test or the
Throughput Test, much less the entire benchmark, doesn't lend itself to
making it easy to evaluate individual query performance.  There may be
times when the developer wants to focus on an individual query to evaluate
the effects of a different index, or database system parameters.

The *run-query* script is intended to allow a developer to generate and
execute one query at a time.  The *dbt7-generate-queries* script can be
used to generate a query file to be executed manually, but *run-query* will
also execute the generated query.
...
`--load` flag.  A PostgreSQL example::
    ...
    dbt7 run --tpcdstools=$DSHOME --load pgsql /tmp/load-results

Then any query can be tested.  For example running Query 55::

    dbt7 run-query --tpcdstools=$DSHOME 55 pgsql
